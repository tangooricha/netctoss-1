...
<% 
	...
	request.removeAttribute("hasSaveModify");
%>
">
	<!--Logo区域开始-->
	<div id="header">
	...
	<div id="main">
		...
		<form action="saveModifyCost.do" method="post" class="main_form">
			<div class="text_info clearfix">
				<span>资费ID：</span>
			</div>
			<div class="input_info">
				<input type="text" class="readonly" readonly name="costId"
					value="${cost.costId}" />
			</div>
			<div class="text_info clearfix">
				<span>资费名称：</span>
			...
			<div class="input_info fee_type">
				<input type="radio" name="radFeeType" value="1"
					<c:if test="${cost.costType==1}">checked="checked"</c:if>
					id="monthly" onclick="feeTypeChange(1);" /> <label for="monthly">包月</label>
				<input type="radio" name="radFeeType" value="2"
					<c:if test="${cost.costType==2}">checked="checked"</c:if>
					id="package" onclick="feeTypeChange(2);" /> <label for="package">套餐</label>
				<input type="radio" name="radFeeType" value="3"
					<c:if test="${cost.costType==3}">checked="checked"</c:if>
					id="timeBased" onclick="feeTypeChange(3);" /> <label
			...
			<div class="button_info clearfix">
				<input type="submit" value="保存" class="btn_save" /> <input
					...
					onclick="location.href='findCost.do'" />
			</div>
		</form>
	</div>
	<!--主要区域结束-->

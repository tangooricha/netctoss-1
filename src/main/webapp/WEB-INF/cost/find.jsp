...
<script language="javascript" type="text/javascript">
			...
			$(function(){
				...
				if(orderType == 'asc')
					return;
				if(orderName=='name'){
					$("[value='月租']").get(0).className='sort_desc';
				} else if(orderName=='base_duration'){
					$("[value='时长']").get(0).className='sort_desc';
				} else if(orderName=='base_cost'){
					$("[value='基费']").get(0).className='sort_desc';
				}
			});
		</script>
</head>
		...
		<form action="" method="">
			...
			<div class="search_add">
				<div>
					<input type="button" value="月租" class="sort_asc"
						...
						value="基费" class="sort_asc" onclick="sort(this);" />
				</div>
			...
			<div id="operate_result_info" class="operate_success">
				<img src="images/close.png"
					onclick="this.parentNode.style.display='none';" /> 操作成功！
			</div>
			<!--数据区域：用表格展示数据-->
			<div id="data">
				<table id="datalist">
						...
						<c:forEach items="${costs}" var="c">
							<tr>
								...
								<td><fmt:formatDate value="${c.startime}"
										pattern="yyyy年MM月dd日 HH:mm:ss" /></td>
								<td><c:if test="${c.status==1}">暂停</c:if> <c:if
										test="${c.status==0}">开通</c:if></td>
								<td><c:if test="${c.status!=0}">
										<input type="button" value="启用" class="btn_start"
											onclick="startFee(this);" />
										<input type="button" value="修改" class="btn_modify"
											onclick="location.href='toModifyCost.do?costId=${c.costId}';" />
										<input type="button" value="删除" class="btn_delete"
											onclick="deleteFee(${c.costId});" />
									</c:if></td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<div id="pages">
				<a
					href="findCost.do?pageNum=1&orderName=${orderName}&orderType=${orderType}">首页</a>
				<c:if test="${currPage!=1}">
					<a
						href="findCost.do?pageNum=${currPage-1}&orderName=${orderName}&orderType=${orderType}">上一页</a>
				</c:if>
				<c:if test="${currPage==1}">
					<a href="">上一页</a>
				</c:if>
				<c:forEach begin="${startPage}" end="${endPage}" var="i">
					...
					<a
						href="findCost.do?pageNum=${i}&orderName=${orderName}&orderType=${orderType}"
						<c:if test="${currPage==i}">class="current_page"</c:if>>${i}</a>

				</c:forEach>
				<c:if test="${currPage!=maxPage}">
					<a
						href="findCost.do?pageNum=${currPage+1}&orderName=${orderName}&orderType=${orderType}">下一页</a>
				</c:if>
				<c:if test="${currPage==maxPage}">
					<a href="">下一页</a>
				</c:if>
				<a
					href="findCost.do?pageNum=${maxPage}&orderName=${orderName}&orderType=${orderType}">尾页</a>
			</div>
		</form>
	</div>
	<!--主要区域结束-->
